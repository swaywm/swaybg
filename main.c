	...
	while (wl_display_dispatch(state.display) != -1 && state.run_display) {
		...
		wl_list_for_each(image, &state.images, link) {
			...
			if (suffix && (!strcmp(suffix, ".svg") || !strcmp(suffix, ".SVG"))) {
				...
				GFile *file = g_file_new_for_path(image->path);

				GFileInputStream* input = g_file_read(file, NULL, &error);
				if (!input) {
					swaybg_log(LOG_ERROR, "Failed to open image: %s, %s",
						image->path, error->message);
					g_object_unref(file);
					g_error_free(error);
					continue;
				}
				svg = rsvg_handle_new_from_stream_sync((GInputStream *)input, NULL,
					RSVG_HANDLE_FLAGS_NONE, NULL, &error);
				if (!svg) {
					g_object_unref(input);
					g_object_unref(file);
					swaybg_log(LOG_ERROR, "Failed to load image: %s, %s",
						image->path, error->message);
					g_error_free(error);
					continue;
				}
